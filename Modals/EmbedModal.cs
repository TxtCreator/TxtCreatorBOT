...
using DisCatSharp.Extensions.Modal.Attributes;
...
public class EmbedModal : ModalCommandModule
{
    ...
    {
        await (await ctx.Guild.GetChannelsAsync()).First(channel => channel.Id == id).SendMessageAsync(_botService.CreateEmbed(title, description, color).WithImageUrl(image).WithThumbnail(thumbnail));
